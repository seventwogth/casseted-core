...
The one place that insensitivity does not hold is content whose own gradient energy is very low, where added contamination can outweigh the signal. That affects chroma on low-saturation fixtures badly enough to invert the apparent hierarchy: on the full render `dark-quiet-floor` reports chroma retention of 0.76 against luma 0.47, while the same fixture measured without contamination reports 0.19 against 0.46.

The suite therefore renders each case twice. Edge retention and the hierarchy assertions are taken from a contamination-free render, because they describe the branch filters; the quiet-region and contamination-character assertions are taken from the full render, because they describe the final pass. Keep that split when adding assertions: ask which render actually carries the property being claimed.
