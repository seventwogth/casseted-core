...
This directory contains the committed visual regression fixtures for the current single-pass still-image pipeline.
...
- `chroma-degradation.png`: reference output for the chroma degradation case
...
The current tests also run small parameter perturbations per stage-oriented case to ensure the fused pass remains responsive but bounded under small control changes.
